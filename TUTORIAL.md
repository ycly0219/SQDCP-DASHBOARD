...
这是一个从「飞书多维表格」读取指标、每日数据与人工问题记录，并渲染成 SQDCP 运营看板的项目。看板按安全（Safety）、质量（Quality）、交付（Delivery）、成本（Cost）、人员（People）五个维度展示每日红绿灯、月度均值、达标率与问题跟踪清单。
...
程序依赖飞书开放平台读取一个多维表格应用，其中必须包含三张表：
...
| 字段 | 说明 | 示例 |
| --- | --- | --- |
| `问题日期` | 问题发生日期，决定问题归属月份 | `2026-08-12` |
| `指标编号` | 对应指标配置表的指标编号 | `D01` |
| `问题描述` | 问题说明 | `出库晚发 3 托` |
| `行动` | 下一步跟进动作，可空；看板为空时显示「—」 | `联系现场确认补发时间` |
...
| `GET /api/data` | 返回当前年份的 `{metrics, daily, issues, months}` JSON |
...
任何返回 `{metrics, daily, issues, months}` 的服务都可以作为数据源，例如 Cloudflare Worker，因此本项目的本地服务也可以替换成远程接口。
...
按 `问题日期` 所在月份以表格展示人工维护的问题，列顺序为问题描述、行动、状态、负责人、计划关闭、日期、指标；`行动` 为空时显示「—」，状态为 `已关闭` 的问题折叠为计数，未配置的指标保留编号并标出「未配置」。

### 指标对比表

按指标主维度计算本月均值，与目标值比较，显示“达标 / 未达标 / 无目标”。
