...
命令幂等：表不存在时创建；表已存在时只补建缺失的「行动」字段。后续人工在飞书「问题跟踪表」中维护问题记录，看板不再自动推导未达标项。
...
- `init-issue-table`：幂等创建或补齐飞书「问题跟踪表」
